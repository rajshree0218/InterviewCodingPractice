...
public class BalanceSheetController {
    public void updateUserExpenseBalanceSheet(User expensePaidBy, List<Split> splits, double totalExpenseAmount){
        ...
        for(Split split : splits) {
            ...
            User userOne = split.getUser();
            UserExpenseBalanceSheet oweUserExpenseSheet = userOne.getUserExpenseBalanceSheet();
            double oweAmount = split.getAmountOne();

            if(expensePaidBy.getUserId().equals(userOne.getUserId())){
                ...
                if(paidByUserExpenseSheet.getUserVsBalance().containsKey(userOne.getUserId())) {

                    userOweBalance = paidByUserExpenseSheet.getUserVsBalance().get(userOne.getUserId());
                }
                else {
                    userOweBalance = new Balance();
                    paidByUserExpenseSheet.getUserVsBalance().put(userOne.getUserId(), userOweBalance);
                }
